...
/**
 * The ruled day grid draws in `--input`, and that is a MEASURED choice.
 *
 ...
 *   --input             3.47 / 3.88   the ≥3:1 boundary tier  ← chosen
 *
 * Nothing between `--secondary-active` and `--input` is a border role, so the pick is `--input`.
 * It must also stay QUIETER than the selected day's --primary fill, so selection is still the
 * loudest shape in the panel.
 */
...
describe.each(THEMES)("calendar grid line ($theme)", ({ selector }) => {
  ...
  const line = rgb("input");

  it.each(["background", "card", "popover", "muted"])("clears 3:1 on --%s", (surface) => {
    expect(contrast(line, rgb(surface))).toBeGreaterThanOrEqual(NON_TEXT);
  });
...
describe("calendar grid line — stylesheet wiring", () => {
  ...
  it("every ruling declaration reads the knob with the --input fallback", () => {
    ...
    for (const b of ruled) {
      for (const decl of b.match(/border-(?:inline|block)-(?:start|end)\s*:[^;]*;/g) ?? []) {
        expect(decl).toContain("var(--calendar-grid-border-color, hsl(var(--input)))");
      }
    }
  });

import { Clock } from "lucide-react";
import { APP_TIME_FORMAT_OPTIONS, getTimeFormatLabel } from "../../app/time-format-labels";
import { useOptionalAppContext } from "../../app/app-provider";
import { useTranslation } from "../../i18n/use-translation";
import { cn } from "../../lib/utils";
import type { TimeFormatPickerProp } from "../../props/components/app.prop";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "../data-entry/select";

export type {
  TimeFormatPickerProp,
  TimeFormatPickerProp as TimeFormatPickerProps,
} from "../../props/components/app.prop";

export function TimeFormatPicker({
  className,
  disabled,
  id,
  value,
  onChange,
}: TimeFormatPickerProp) {
  const ctx = useOptionalAppContext();
  const { t, locale, fallbackLocale } = useTranslation();
  const current = value ?? ctx?.timeFormat;
  const handleChange = onChange ?? ctx?.setTimeFormat;

  if (current === undefined || !handleChange) {
    throw new Error("TimeFormatPicker requires <AppProvider> or controlled value + onChange");
  }

  return (
    <Select value={current} onValueChange={handleChange} disabled={disabled}>
      <SelectTrigger
        id={id}
        className={cn("w-full sm:w-44", className)}
        aria-label={t("navigation.timeFormatPicker.ariaLabel")}
      >
        <Clock className="mr-2 size-4 shrink-0 opacity-70" aria-hidden="true" />
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        {APP_TIME_FORMAT_OPTIONS.map((option) => (
          <SelectItem key={option.value} value={option.value}>
            {getTimeFormatLabel(option.value, locale, fallbackLocale)}
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
  );
}

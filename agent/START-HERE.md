...
3. `rules.json` — 50 cardinal rules. The ones about raw HTML and hardcoded colour are not
   style advice.
4. `tokens.json` — 2072 design tokens, each tagged with its `tier`. **If you were handed a
   ...
   1758 `component` entries are per-part knobs; reach for one only when a role is
...
| `component` | 1758 | per-part knobs, `--{component}-{part}-{property}` | rarely. Most are declared `initial` with the real default at the call site — deliberate, so a scoped override re-resolves instead of freezing at `:root` |

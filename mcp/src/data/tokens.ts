...
export const TOKENS: TokenEntry[] = [
  ...
  },
  {
    ...
    category: "primitive",
    tier: "primitive",
    role: "The AMBIENT/LOOPING interval (1400ms) — the cycle member of the motion tier. `--duration-{fast,base,slow}` time a TRANSITION (A to B, then stop); a loop needs an interval, so every ambient affordance in the product breathes at one rate. `Activity` reads it through `--activity-interval`.",
  },
  {
  ...
  },
  {
    ...
    category: "semantic",
    tier: "semantic",
    role: 'The IDENTITY role — the canonical GoDX emerald (light #009766 = oklch(0.595 0.137 162.94); dark lifts lightness only, #00b87c). It is what `<Logo mark="godx">` / `tone="success">` / the godx wordmark / the `@godxjp/ui/email` brand capsule paint, and it is distinct from THREE lookalikes: --primary (the ACTION colour — re-theme it freely, the mark never moves), --success (若竹 #68be8d, the STATUS green for badges/progress/"saved" text — the mark once borrowed this and shipped ΔE76 ≈ 17.5 off canonical), and --text-brand (the blue brand-NUMERAL text slot behind the Tailwind `text-brand` utility). There is no `bg-brand` utility: `--color-brand` is already taken by --text-brand, so identity surfaces read `hsl(var(--brand))` directly in component CSS. Re-brand a service by overriding --brand once (scoped `[data-tenant]` works — every logo knob resolves it at the call site).',
  },
  {
    name: "--success",
    category: "semantic",
    tier: "semantic",
    role: 'Success STATUS role (若竹 wakatake #68be8d) — badges, alerts, progress fills, timeline done dots, `tone="success"` text. NOT the product identity: the GoDX mark reads --brand.',
  },
  ...
  },
  {
    ...
    category: "semantic",
    tier: "semantic",
    role: "Gap between the page title and its status/meta band (PageContainer `status`), and between the band's own items. Default --space-inline-sm. The band shares the title line and wraps under it on compact viewports; a page that never passes `status` reads neither the token nor the wrapping row.",
  },
  {
  ...
  },
  {
    ...
    category: "component",
    tier: "component",
    role: "Steps type=inline compact auth/device progress rhythm, typography and EMPHASIS. Consumers select the semantic appearance; services retune the row once without page CSS. The step number's emphasis is two knobs — --steps-inline-index-font-weight (default bold) and --steps-inline-index-color (role-mirror `initial`, default the inherited step colour) — because the canonical hosted-identity row marks the step with an accent TINT at normal weight while the library default marks it with bold. --steps-inline-separator-color (role-mirror `initial`, default --muted-foreground) tints the glyph the `separator` prop picks.",
  },
  {

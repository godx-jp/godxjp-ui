  ...
  // ─── layout ─────────────────────────────────────────────────────────────
  {
    ...
    props: [
      ...
      { name: "subtitle", type: "string", description: "Secondary line beneath the title." },
      {
        ...
        type: "ReactNode",
        description:
          'Status/meta band beside the title (StatusBadge, environment tag, "updated …" meta). Sits on the title line at the token-owned --page-header-status-gap and wraps UNDER the title on compact viewports. Part of the canonical page-header contract — never hand-lay a badge next to the <h1>.',
      },
      {
        name: "extra",
        type: "ReactNode",
      ...
      },
      {
        ...
        type: "ReactNode",
        description:
          'The page sections. Every direct child of the body is spaced from the previous one by --page-body-gap (the section step): drop your Cards straight in — do NOT wrap them in a Flex to space them, do NOT add gap-*/mt-*. Group items INSIDE a section with <Flex direction="col" gap> or <ResponsiveGrid>.',
      },
      {
        name: "breadcrumb",
  ...
  },
  {
    ...
    props: [
      ...
      },
      {
        ...
        description:
          "DEPRECATED — use linkComponent, or asChild on SidebarItem. Legacy escape hatch that left row CONTENT to the caller, which is how a <Link>{item.label}</Link> silently dropped every icon and badge in production. Still supported and still wins over linkComponent; rowProps now also carries the library-composed children, so spreading rowProps (or rendering rowProps.children) restores the canonical row.",
      },
      {
        name: "aria-label",
        type: "string",
  ...
  },
  {
    ...
    props: [
      ...
      },
      {
        ...
        description:
          "Font weight — the 3-weight canon: regular 400 (body), medium 500 (label), bold 700 (emphasis). `semibold` is an alias that renders at the canon's 500 (there is no 600 face), so code written in the common Tailwind vocabulary type-checks without a fourth weight.",
      },
  ...
  },
  {
    ...
    props: [
      ...
      },
      {
        ...
        defaultValue: '"full"',
        description:
          "Compound API only. `full` fills the field column (inside FormField). `auto` sizes the trigger to its label — use it in a PageContainer `extra` slot, a toolbar or a footer row, where a full-width trigger swallows the row and truncates its siblings.",
      },
      {
  ...
  },
  {
    ...
    props: [
      ...
      },
      {
        ...
        defaultValue: "false",
        description:
          "Pin the strip to the top of its scroll container while the list scrolls beneath it. Opt-in; tune offset/fill via the --filter-bar-sticky-offset / --filter-bar-sticky-background theme knobs.",
      },
      {
        name: "overflow",
        type: "'wrap' | 'scroll'",
  ...
  },
  {
    ...
    props: [
      ...
      },
      {
        ...
        defaultValue: "false",
        description:
          'Compact trigger density: re-tiers the box to the official --control-height-sm tier and DROPS the picker\'s owned per-kind width, so a labelled trigger hugs its value. This is the supported auth/legal-footer locale switch — `<AppSettingPicker kind="locale" appearance="labeled" compact />` — when the square icon-only default reads as a stray button but the full labelled trigger is too tall. All geometry is tokenized (--app-setting-picker-compact-{control-height,padding-x,gap,font-size}); the accessible name and the visible value are both preserved. No effect on appearance="inline", which is already chrome-less.',
      },
      {
        name: "className",
        type: "string",

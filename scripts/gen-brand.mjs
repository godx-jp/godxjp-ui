...
async function loadContrastHelpers() {
  ...
  execFileSync(
    ...
    [
      join(ROOT, "src/tokens/__tests__/wcag-contrast.ts"),
      "--bundle",
      "--format=esm",
      "--platform=node",
      "--packages=external",
...
const themeBlock = (selector, seed, label, text, themeDefault, note) => {
  const lines = [
    `  /* ${note} */`,
  ...
  lines.push(
    "",
    "  /* Brand text. One ramp step off the action colour, and the pressed step — the same",
    "   * formulas derived.css uses, so these move when the seed does. */",
    ...Object.entries(text).map(
      ([role, value]) => `  ${role}: ${asTriplet(value)}; /* ${toHex(value)} */`,
    ),
  );
  return `${selector} {\n${lines.join("\n")}\n}`;
};
...
${themeBlock(":root", lightSeed, lightLabel, lightText, "darken", "Light (default)")}

${themeBlock('.dark,\n:root[data-theme="dark"]', darkSeed, darkLabel, darkText, "lighten", `Dark — the seed lifted ${(darkSeed[2] - lightSeed[2]).toFixed(1)}% in lightness, hue and saturation held`)}
`;

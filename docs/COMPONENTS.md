...
**Prop type:** `PageHeaderProp` 
**Import:** `@godxjp/ui/layout`

The page title band PageContainer renders internally, exported for surfaces that are **not** a whole
page (a Sheet detail, a `MasterDetail` pane, a tab body). There is one implementation, so the two
cannot drift.
...
| `extra`      | `ExtraProp`         |                 |
| `breadcrumb` | `BreadcrumbProp`    |                 |
| `layout`     | local               | default `stack` |
| `loading`    | local (`boolean`)   | default `false` |

Never nest it inside PageContainer (two `<h1>`s), and mount it inside sectioning content so its
`<header>` does not compute to a second `banner` landmark. Full contract:
[docs/CANONICAL-CONTRACTS.md](./CANONICAL-CONTRACTS.md).
...
| Component        | Prop type          | Key props                                              |
| ---------------- | ------------------ | ------------------------------------------------------ |
| `FilterBar`      | `ToolbarProp`      | `search`, `chips`, `resultCount`, `actions`, `onClear` |

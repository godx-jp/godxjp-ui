...
PrefetchLink — Primitive trạng thái React Query — loading, lỗi, refetch.
...
| ------------ | -------------------------------- | ------------ | ------------------ | --------------------------------------------------------- |
| `queryKey`   | `QueryKey`                       | **bắt buộc** | Prop `queryKey`.   | Tham chiếu type trong source `src/props/` hoặc component. |
| `queryFn`    | `() => Promise<unknown>`         | **bắt buộc** | Prop `queryFn`.    | Tham chiếu type trong source `src/props/` hoặc component. |
| `prefetchOn` | `hover`, `focus`, `both`, `none` | —            | Prop `prefetchOn`. | Tham chiếu type trong source `src/props/` hoặc component. |
| `staleTime`  | `number`                         | —            | Prop `staleTime`.  | Tham chiếu type trong source `src/props/` hoặc component. |

...
Progress — Primitive hiển thị dữ liệu — chỉ đọc, không nhập liệu.
...
| ------- | ------------------- | ------------ | ------------------------------------------ | -------------------------------------------------------------- |
| `value` | `number`            | **bắt buộc** | Giá trị hiển thị chính.                    | StatCard: số KPI; Select controlled: option đang chọn.         |
| `label` | `string`            | —            | Nhãn mô tả — thường là dòng phụ, font nhỏ. | StatCard: tên chỉ số KPI; FormField: tên field.                |
| `tone`  | `ProgressTone` | —            | Tông màu semantic.                         | Badge: success/warning/destructive theo business status. |

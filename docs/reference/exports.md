...
| `@godxjp/ui/components/primitives` | `dist/components/primitives.d.ts` | `dist/components/primitives.js` | All 61 primitive components, re-exported from the six group folders (`general` / `layout` / `data-display` / `data-entry` / `feedback` / `navigation`) via `src/components/primitives.ts` |

...
Use it for read-only profile / record cards. For editable forms use [Form](../data-entry/Form.md) + [Field](../data-entry/Field.md); for sortable / filterable tabular data use [Table](./Table.md).
...
- For mission-critical pairings (legal IDs, audit timestamps) wrap individual items in `<dl>` manually via the `...rest` spread, or pair with [Field](../data-entry/Field.md) when the grid contains a mix of static and editable rows.
...
- [Form](../data-entry/Form.md) — editable equivalent.
- [Field](../data-entry/Field.md) — single label + value + help group.

...
Compositional — render `<CollapsePanel value="…" title="…">` children. The framework owns expanded-state via React context; each panel reads `aria-expanded` and renders / hides its content region. The chevron rotates via `data-state="open"` on the panel root.
...
| Tabbed switching between mutually exclusive content | [Tabs](./Tabs.md) |
| Modal / sheet-style detail surface | [Dialog](./Dialog.md) / [Sheet](./Sheet.md) |
...
```tsx
import { Collapse, CollapsePanel } from "@godxjp/ui"
...
| `children` | `ReactNode` | — | `<CollapsePanel>` children |
...
<Collapse variant="outlined" defaultValue="q1">
  {FAQ.map((q) => (
    <CollapsePanel key={q.value} value={q.value} title={q.title} extra={<span>必読</span>}>
      <p>{q.body}</p>
    </CollapsePanel>
  ))}
</Collapse>
...
function ControlledCollapse() {
  ...
  return (
    <Collapse value={open} onValueChange={(v) => setOpen(v as string)}>
      <CollapsePanel value="q1" title="Question 1">…</CollapsePanel>
      <CollapsePanel value="q2" title="Question 2">…</CollapsePanel>
    </Collapse>
  )
}
```
...
- [Tabs](./Tabs.md) — mutually exclusive section switching.

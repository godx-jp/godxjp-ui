...
| Single high-stakes confirmation in flow | [Popconfirm](./Popconfirm.md) |

Tour is a *guided sequence* — Tooltip / Popover surface a single hint at user request. If you find yourself rendering three Tooltips one after another, reach for Tour.
...
<Tour
  defaultOpen
  steps={[
    { target: "#new-btn",     title: "新規作成", description: "ここから新しいレコードを作成できます。" },
    { target: "#filter-btn",  title: "絞り込み", description: "条件を指定して一覧を絞り込みます。" },
    { target: "#settings-btn", title: "設定",   description: "個人設定とテーマを変更できます。" },
  ]}
/>
```
...
function ControlledTour() {
  ...
  return (
    <>
      <Button onClick={() => { setStep(0); setOpen(true) }}>ツアー開始</Button>
      <Tour
        open={open}
...
<Tour
  ...
  steps={[
    { title: "ようこそ", description: "簡単な紹介をお見せします。", placement: "center" },
    { title: "ダッシュボード", description: "重要な指標がひと目で分かります。", placement: "center" },
    { title: "準備完了", description: "それでは始めましょう。", placement: "center" },
  ]}
/>
```
...
- [Dialog](./Dialog.md) / [AlertDialog](./AlertDialog.md) — modal alternative when the flow demands confirmation, not introduction.

...
> Header + items + optional footer surface. Render data-driven (`dataSource` + `renderItem`) or compositional (`<ListItem>` children); switch to a grid by setting `cols > 1`.
...
| Flat menu of links / nav items | [Menu](./Menu.md) |

## Usage

```tsx
import { List, ListItem, Avatar, Tag } from "@godxjp/ui"
...
<List
  ...
  renderItem={(emp) => (
    <ListItem
      ...
      extra={<Tag color={statusColor[emp.status]}>{statusLabel[emp.status]}</Tag>}
    />
  )}
/>
```
...
| `children` | `ReactNode` | — | Compositional `<ListItem>` children when `dataSource` is omitted |
...
`ListItem` IS the `<li>` — when you supply `renderItem`, the framework does not wrap its return in another `<li>` (nested `<li>` is invalid HTML). Return a `<ListItem>` (or another `<li>`) from `renderItem`; the framework injects the React `key` via `cloneElement`.
...
- [Pagination](./Pagination.md) — fits the `footer` slot.

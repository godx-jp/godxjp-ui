...
- **Row/Col** is a *grid* — children sum to ≤ 24 columns; breakpoint-aware via `xs`/`sm`/`md`/`lg`/`xl`/`xxl` overrides on `Col`.
- **Flex** is a *flex container* — one axis at a time, prop-driven.
- **Space** is an *inline group* — the same shape as Flex but the semantics ("inline-flex") signal "small adjacent items", not "section layout".

...
> Image / content slide deck. Compositional — pass `<CarouselSlide>` children, one per slide.
...
| See all items at once at a glance | [List](./List.md) `cols={N}` or a [Grid](./Grid.md) |
...
```tsx
import { Carousel, CarouselSlide } from "@godxjp/ui"
...
| `children` | `ReactNode` | — | `<CarouselSlide>` children |
...
<Carousel autoplay={3000}>
  {slides.map((s, i) => (
    <CarouselSlide key={i}>{s.content}</CarouselSlide>
  ))}
</Carousel>
...
<div style={{ maxWidth: 480, height: 320 }}>
  <Carousel orientation="vertical">
    {slides.map((s, i) => <CarouselSlide key={i}>{s.content}</CarouselSlide>)}
  </Carousel>
</div>

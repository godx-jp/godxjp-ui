...
Built on top of [Popover](../data-display/Popover.md): the trigger renders via Radix `asChild`, and the popover body holds a title, optional description, optional leading icon, plus Cancel / Confirm buttons. The default icon colour matches `confirmVariant` — amber `--warning` for `"primary"`, red `--destructive` for `"destructive"`.
...
- [Popover](../data-display/Popover.md) — underlying primitive without the confirm scaffolding.

...
/* The halo is .ui-brand-glow (src/styles/layout.css), which SHIPS and was going unused. These
   two classes now only place it and set its four documented knobs, instead of re-writing the
   radial-gradient the framework already composes. */
[data-tenant="acme-web"] .tx-glow-tr,
[data-tenant="acme-web"] .tx-glow-bl { position: absolute; inset: 0; --brand-glow-color: 41 73% 53%; --brand-glow-size: 40% 50%; }
[data-tenant="acme-web"] .tx-glow-tr { --brand-glow-position: 88% -10%; --brand-glow-alpha: 0.22; }
[data-tenant="acme-web"] .tx-glow-bl { --brand-glow-position: 12% 120%; --brand-glow-alpha: 0.18; }
[data-tenant="acme-web"] .tx-field { display: flex; align-items: center; gap: var(--space-3); padding: var(--space-3) var(--space-4);
...
function Hero() {
  ...
  return (
    <section className="tx-navy relative overflow-hidden">
      <div className="ui-brand-glow tx-glow-tr" />
...
function CtaBanner() {
  return (
    <section className="tx-navy tx-section relative overflow-hidden">
      <div className="ui-brand-glow tx-glow-bl" />
